...
// insert functions
template <class T>
int insert(T& tcm, string filename, int type, int data_interval);
...
template <class T>
uint32_t allEdgeFrequenceQuery(T& egss, string input_dir, string output_dir, string dataset_name, bool write, uint32_t n, int fun);
template <class T>
...
template <class T>
int insert(T& tcm, string filename, int type, int data_interval) {		// int data_interval - granularity = 10000
	ifstream ifs;
	ifs.open(filename);
	if (!ifs.is_open()) {
...
template <class T>
int insert_tp_time(T& tcm, string filename, int type) {					// int data_interval - granularity = 10000
	ifstream ifs;
	ifs.open(filename);
	if (!ifs.is_open()) {
		cout << "Error in open file, Path = " << filename << endl;
